...
* @throws ApiError
*/
export const useSongServiceSongsIndexSuspense = <TData = Common.SongServiceSongsIndexDefaultResponse, TError = unknown, TQueryKey extends Array<unknown> = unknown[]>({ genreNames, genreSlugs, library, page, perPage, relations }: {
  genreNames?: string;
  genreSlugs?: string;
  library: string;
  page?: number;
  perPage?: number;
  relations?: string;
}, queryKey?: TQueryKey, options?: Omit<UseQueryOptions<TData, TError>, "queryKey" | "queryFn">) => useSuspenseQuery<TData, TError>({ queryKey: Common.UseSongServiceSongsIndexKeyFn({ genreNames, genreSlugs, library, page, perPage, relations }, queryKey), queryFn: () => SongService.songsIndex({ genreNames, genreSlugs, library, page, perPage, relations }) as TData, ...options });
/**
* Get a song
* @param data The data for the request.
* @param data.library The library slug
