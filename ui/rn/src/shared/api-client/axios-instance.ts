/**
 * RN Axios instance with DPoP interceptors.
 *
 * Same pattern as the web axios-instance, but:
 * - Uses useAuthStore (RN version)
 * - Gets baseURL from auth store (serverUrl, not window.__BAANDER_API_URL__)
 * - No window.location references
...
export const AXIOS_INSTANCE = Axios.create({
  ...
  headers: {
    ...
    'User-Agent': 'Baander/Client(edison)/1.0',
  },
});
