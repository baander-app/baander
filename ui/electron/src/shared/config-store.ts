...
export function initMainProcessImpl(
  ...
  try {
    ...
    _mainProcessImpl = {
      ...
      getThemeMood: (): string => {
        const config = _mainProcessImpl.loadConfig();
        return config.themeMood || 'dark';
      },
      setThemeMood: (mood: string): void => {
        const config = _mainProcessImpl.loadConfig();
        config.themeMood = mood;
...
export function getThemeMoodSync(): string {
  if (_mainProcessImpl) {
    return _mainProcessImpl.getThemeMood();
  }
  return 'dark';
}
