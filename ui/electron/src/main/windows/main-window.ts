...
import { getThemeMoodSync } from '../../shared/config-store';
...
export function createMainWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    backgroundColor: { dark: '#000000', warm: '#faf5ee', cool: '#f0f4f8', balanced: '#f5f5f5' }[getThemeMoodSync()] || '#000000',
    width: 1200,
    height: 800,
    show: false,
    // Custom titlebar: macOS preserves native traffic lights, Windows/Linux uses frameless
    titleBarStyle: process.platform === 'darwin' ? 'hidden' : undefined,
    ...(process.platform !== 'darwin' ? { frame: false } : {}),
    webPreferences: {
      preload: app.isPackaged
        ? join(__dirname, 'preload.cjs')  // Production: same directory (flattened)
        : join(__dirname, '../preload/preload.cjs'),  // go up from dist/main to dist, then preload/
