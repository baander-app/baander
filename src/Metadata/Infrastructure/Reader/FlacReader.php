...
/**
 * Reads FLAC metadata by parsing the binary file directly.
 *
 * Uses FlacParser for native STREAMINFO, VORBIS_COMMENT, and
 * METADATA_BLOCK_PICTURE extraction. No external commands required.
 */
final class FlacReader
{
    ...
    public function read(string $path): ExtractedMetadata
    {
        if (!is_file($path) || !is_readable($path)) {
            $this->logger->warning('FLAC file does not exist or is not readable', ['path' => $path]);

            return new ExtractedMetadata();
        }

        try {
            $parser = new FlacParser($path, $this->logger);
            $parser->parse();
        } catch (\Throwable $e) {
            $this->logger->warning('Failed to parse FLAC file', [
                'path' => $path,
                'error' => $e->getMessage(),
            ]);
    ...
    private function mapToMetadata(FlacParser $parser): ExtractedMetadata
    {
        $metadata = new ExtractedMetadata();

        // Stream info
        $streamInfo = $parser->getStreamInfo();
        if ($streamInfo !== null) {
            $metadata->setSampleRate($streamInfo['sampleRate']);
            $metadata->setChannels($streamInfo['channels']);
            ...
            if ($streamInfo['totalSamples'] > 0 && $streamInfo['sampleRate'] > 0) {
                $metadata->setDuration(
                    (float) ($streamInfo['totalSamples'] / $streamInfo['sampleRate']),
                );
            }
        }
        ...
        if ($commentBlock !== null) {
            ...
            // Only set duration from VORBIS comment if not already set from STREAMINFO
        ...
        // Pictures
        $pictures = [];
        foreach ($parser->getPictureBlocks() as $pictureData) {
            if ($pictureData !== []) {
