...
use SwooleBundle\SwooleBundle\Bridge\Log\JsonAccessLogFormatter;
...
final class SwooleExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        // Register watchdog services — always available (uses error_log, not pooled logger)
        $watchdogDef = new Definition(\SwooleBundle\SwooleBundle\Coroutine\Watchdog\CoroutineWatchdog::class);
        $watchdogDef->setArgument('$stallThresholdMs', 30_000);
        $container->setDefinition(\SwooleBundle\SwooleBundle\Coroutine\Watchdog\CoroutineWatchdog::class, $watchdogDef);

        $handlerDef = new Definition(\SwooleBundle\SwooleBundle\Server\WorkerHandler\CoroutineWatchdogWorkerStartHandler::class);
        $handlerDef->setArgument('$watchdog', new Reference(\SwooleBundle\SwooleBundle\Coroutine\Watchdog\CoroutineWatchdog::class));
        $handlerDef->setArgument('$swoole', new Reference(Swoole::class));
        $handlerDef->setArgument('$intervalMs', 5000);
        $handlerDef->setArgument('$decorated', new Reference(\SwooleBundle\SwooleBundle\Server\WorkerHandler\CoroutineWatchdogWorkerStartHandler::class . '.inner'));
        $handlerDef->setDecoratedService(WorkerStartHandler::class, null, 10);
        $container->setDefinition(\SwooleBundle\SwooleBundle\Server\WorkerHandler\CoroutineWatchdogWorkerStartHandler::class, $handlerDef);
    ...
    private function configureHttpServerServices(array $config, ContainerBuilder $container): void
    {
        ...
        if ($config['access_log']['format'] === 'json') {
            $accessLogFormatter->setClass(JsonAccessLogFormatter::class);
        } else {
            $accessLogFormatter->setClass(SimpleAccessLogFormatter::class);
    ...
    private function configureExceptionHandler(array $config, ContainerBuilder $container): void
    {
        ...
        if ($verbosity === 'auto') {
            ...
            } else if ($this->isDebug($container)) {
                $verbosity = 'trace';
            } else {
                $verbosity = 'verbose';
            }
    ...
    ): void
    {
        ...
        // Tell the Swoole wrapper whether thread mode is configured
        $container->getDefinition(Swoole::class)
            ->addMethodCall('setThreadMode', [$runningMode === 'thread']);
    }
