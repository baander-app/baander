...
Outputs:
  ...
  [vector_real] autoCorrelation - the autocorrelation vector
...
Parameters:
  ...
  frequencyDomainCompression:
    ...
    factor at which FFT magnitude is compressed (only used if 'generalized' is
    set to true, see [3])

  generalized:
    bool ∈ {true,false} (default = false)
    bool value to indicate whether to compute the 'generalized' autocorrelation
    as described in [3]

  normalization:
    string ∈ {standard,unbiased} (default = "standard")
...
Description:
  ...
  It uses the version most commonly used in signal processing, which doesn't
  remove the mean from the observations.
  Using the 'generalized' option this algorithm computes autocorrelation as
  described in [3].
  ...
  multipitch analysis model.
    IEEE Transactions on Audio, Speech, and Language Processing, 8(6), 708-716.
 * 
 * Category: Stats
 * Mode: standard
 */
class AutoCorrelation extends BaseAlgorithm
{
    protected string $algorithmName = 'AutoCorrelation';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute AutoCorrelation: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create AutoCorrelation algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing AutoCorrelation: " . $e->getMessage(), 0, $e);
        }
    }
        ...
        switch ($this->category) {
            ...
            case 'Tonal':
                if (!is_array($input) && !($input instanceof AudioVector)) {
                    throw new AlgorithmException('Stats algorithms expect array or AudioVector input');
                }
                break;
