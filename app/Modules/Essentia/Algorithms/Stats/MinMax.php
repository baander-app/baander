...
Outputs:
  ...
  [integer] int - the index of the value


Parameters:

  type:
    string ∈ {min,max} (default = "min")
...
Description:
  ...
  If the array has more than one minimum or maximum value, the index of the
  first one is returned
 * 
 * Category: Stats
 * Mode: standard
 */
class MinMax extends BaseAlgorithm
{
    protected string $algorithmName = 'MinMax';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute MinMax: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create MinMax algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing MinMax: " . $e->getMessage(), 0, $e);
        }
    }
