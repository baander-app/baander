...
Inputs:
  ...
  [vector_stereosample] audio - the audio signal
...
Description:
  ...
  The algorithm uses the FFmpeg library. Supported formats are wav, aiff, mp3,
  flac and ogg. The default FFmpeg encoders are used for each format.
  
  An exception is thrown when other extensions are given. Note that to encode
  in mp3 format it is mandatory that FFmpeg was configured with mp3 enabled.
 * 
 * Category: Io
 * Mode: standard
 */
class AudioWriter extends BaseAlgorithm
{
    protected string $algorithmName = 'AudioWriter';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute AudioWriter: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create AudioWriter algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing AudioWriter: " . $e->getMessage(), 0, $e);
        }
    }
    ...
    private function getValidParameters(): array
    {
        ...
        return ['filename', 'format', 'sampleRate'];
    }
}