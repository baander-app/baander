...
class MonoLoader extends BaseAlgorithm
{
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create MonoLoader algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing MonoLoader: " . $e->getMessage(), 0, $e);
        }
    }
    ...
    private function getValidParameters(): array
    {
        ...
        return ['filename', 'sampleRate', 'downmix'];
    }
}