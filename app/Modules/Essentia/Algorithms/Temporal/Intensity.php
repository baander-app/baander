...
Outputs:
  ...
  [integer] intensity - the intensity value
...
Description:
  ...
  This algorithm classifies the input audio signal as either relaxed (-1),
  moderate (0), or aggressive (1).
  
  Quality: outdated (non-reliable, poor accuracy).
  
  An exception is thrown if empty input is provided because the "intensity" is
  not defined for that case.
 * 
 * Category: Temporal
 * Mode: standard
 */
class Intensity extends BaseAlgorithm
{
    protected string $algorithmName = 'Intensity';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute Intensity: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create Intensity algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing Intensity: " . $e->getMessage(), 0, $e);
        }
    }
