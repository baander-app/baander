...
Outputs:
  ...
  [vector_real] bands - the energies/magnitudes of each band
...
Description:
  ...
  This algorithm computes energies/magnitudes in ERB bands of a spectrum. The
  Equivalent Rectangular Bandwidth (ERB) scale is used. The algorithm applies a
  frequency domain filterbank using gammatone filters. Adapted from matlab code
  in:  D. P. W. Ellis (2009). 'Gammatone-like spectrograms', web resource [1].
 ...
 * Mode: standard
 */
class ERBBands extends BaseAlgorithm
{
    protected string $algorithmName = 'ERBBands';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute ERBBands: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create ERBBands algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing ERBBands: " . $e->getMessage(), 0, $e);
        }
    }
    ...
    private function getValidParameters(): array
    {
        ...
        return ['sampleRate', 'inputSize', 'numberBands', 'lowFrequencyBound', 'highFrequencyBound', 'normalize', 'type', 'width'];
    }
}