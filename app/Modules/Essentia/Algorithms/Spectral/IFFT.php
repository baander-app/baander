...
Outputs:
  ...
  [vector_real] frame - the IFFT of the input frame
...
Description:
  ...
  size of (s-1)*2, where s is the size of the input fft frame. The inverse
  Fourier transform is not defined for frames which size is less than 2
  samples. Otherwise an exception is thrown.
 ...
 * Mode: standard
 */
class IFFT extends BaseAlgorithm
{
    protected string $algorithmName = 'IFFT';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute IFFT: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create IFFT algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing IFFT: " . $e->getMessage(), 0, $e);
        }
    }
