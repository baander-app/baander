...
Description:
  ...
  This algorithm estimates the fundamental frequency corresponding to the
  melody of a monophonic music signal based on the MELODIA algorithm. While the
  algorithm is originally designed to extract the predominant melody from
  polyphonic music [1], this implementation is adapted for monophonic signals.
  The approach is based on the creation and characterization of pitch contours,
  time continuous sequences of pitch candidates grouped using auditory
  streaming cues. To this end, PitchSalienceFunction,
  PitchSalienceFunctionPeaks, PitchContours, and PitchContoursMonoMelody
  ...
  provided) except for minFrequency and maxFrequency, which will depend on your
  application.
 ...
 * Mode: standard
 */
class PitchMelodia extends BaseAlgorithm
{
    protected string $algorithmName = 'PitchMelodia';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute PitchMelodia: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create PitchMelodia algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing PitchMelodia: " . $e->getMessage(), 0, $e);
        }
    }
