...
Outputs:
  ...
  [pool] pool - pool with classification results (resulting from the transformation of the gaia point)
...
Parameters:
  ...
  svms:
    vector_string
    list of svm models (gaia2 history) filenames.


Description:

  This algorithms computes SVM predictions given a pool with aggregated
  descriptor values computed by MusicExtractor (or FreesoundExtractor).
 * 
 * Category: Extractor
 * Mode: standard
 */
class MusicExtractorSVM extends BaseAlgorithm
{
    protected string $algorithmName = 'MusicExtractorSVM';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute MusicExtractorSVM: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create MusicExtractorSVM algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing MusicExtractorSVM: " . $e->getMessage(), 0, $e);
        }
    }
