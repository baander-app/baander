...
Outputs:
  ...
  [real] maxToTotal - the maximum amplitude position to total length ratio


Description:

  This algorithm computes the ratio between the index of the maximum value of
  the envelope of a signal and the total length of the envelope. This ratio
  shows how much the maximum amplitude is off-center. Its value is close to 0
  if the maximum is close to the beginning (e.g. Decrescendo or Impulsive
  sounds), close to 0.5 if it is close to the middle (e.g. Delta sounds) and
  close to 1 if it is close to the end of the sound (e.g. Crescendo sounds).
 ...
 * Mode: standard
 */
class MaxToTotal extends BaseAlgorithm
{
    protected string $algorithmName = 'MaxToTotal';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute MaxToTotal: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create MaxToTotal algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing MaxToTotal: " . $e->getMessage(), 0, $e);
        }
    }
        ...
        switch ($this->category) {
            ...
            case 'Tonal':
                if (!is_array($input) && !($input instanceof AudioVector)) {
                    throw new AlgorithmException('Sfx algorithms expect array or AudioVector input');
                }
                break;
