...
Outputs:
  ...
  [vector_real] dct - the discrete cosine transform of the input array
...
Description:
  ...
  It uses the DCT-II form, with the 1/sqrt(2) scaling factor for the first
  coefficient.
  
  Note: The 'inputSize' parameter is only used as an optimization when the
  algorithm is configured. The DCT will automatically adjust to the size of any
  input.
 ...
 * Mode: standard
 */
class DCT extends BaseAlgorithm
{
    protected string $algorithmName = 'DCT';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute DCT: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create DCT algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing DCT: " . $e->getMessage(), 0, $e);
        }
    }
