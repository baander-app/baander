...
Inputs:
  ...
  [vector_complex] fft - the input fft
            [real] pitch - external pitch input [Hz].
...
Description:
  ...
  This algorithm uses SineModelAnal and keeps only the harmonic partials. It
  receives an external pitch value as input. You can use PitchYinFft algorithm
  to compute the pitch per frame.
 ...
 * Mode: standard
 */
class HarmonicModelAnal extends BaseAlgorithm
{
    protected string $algorithmName = 'HarmonicModelAnal';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute HarmonicModelAnal: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create HarmonicModelAnal algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing HarmonicModelAnal: " . $e->getMessage(), 0, $e);
        }
    }
        ...
        switch ($this->category) {
            ...
            case 'Tonal':
                if (!is_array($input) && !($input instanceof AudioVector)) {
                    throw new AlgorithmException('Synthesis algorithms expect array or AudioVector input');
                }
                break;
    ...
    private function getValidParameters(): array
    {
        ...
        return ['sampleRate', 'maxnSines', 'magnitudeThreshold', 'minSineDur', 'nHarmonics', 'harmThreshold'];
    }
}