...
Inputs:
  ...
  [vector_vector_real] observationProbabilities - the observation probabilities
         [vector_real] initialization - the initialization
      [vector_integer] fromIndex - the transition matrix from index
      [vector_integer] toIndex - the transition matrix to index
         [vector_real] transitionProbabilities - the transition probabilities matrix


Outputs:

  [vector_integer] path - the decoded path


Description:

  This algorithm estimates the most-likely path by Viterbi algorithm. It is
  used in PitchYinProbabilistiesHMM algorithm.
  
  This Viterbi algorithm returns the most likely path. The internal variable
  calculation uses double for a better precision.
 ...
 * Mode: standard
 */
class Viterbi extends BaseAlgorithm
{
    protected string $algorithmName = 'Viterbi';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute Viterbi: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create Viterbi algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing Viterbi: " . $e->getMessage(), 0, $e);
        }
    }
        ...
        switch ($this->category) {
            ...
            case 'Tonal':
                if (!is_array($input) && !($input instanceof AudioVector)) {
                    throw new AlgorithmException('MachineLearning algorithms expect array or AudioVector input');
                }
                break;
