...
Outputs:
  ...
  [vector_real] onsets - the positions of detected onsets [s]
         [real] onsetRate - the number of onsets per second


Description:

  This algorithm computes the number of onsets per second and their position in
  time for an audio signal. Onset detection functions are computed using both
  high frequency content and complex-domain methods available in OnsetDetection
  algorithm. See OnsetDetection for more information.
  Please note that due to a dependence on the Onsets algorithm, this algorithm
  is only valid for audio signals with a sampling rate of 44100Hz.
  This algorithm throws an exception if the input signal is empty.
 * 
 * Category: Rhythm
 * Mode: standard
 */
class OnsetRate extends BaseAlgorithm
{
    protected string $algorithmName = 'OnsetRate';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute OnsetRate: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create OnsetRate algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing OnsetRate: " . $e->getMessage(), 0, $e);
        }
    }
