...
Inputs:
  ...
  [vector_vector_real] bands - the input bands spectrogram


Outputs:

  [real] differences - SuperFlux novelty curve


Parameters:

  binWidth:
    integer ∈ [3,inf) (default = 3)
    filter width (number of frequency bins)

  frameWidth:
    integer ∈ (0,inf) (default = 2)
    differentiation offset (compute the difference with the N-th previous
    frame)


Description:

  Onset detection function for Superflux algorithm. See SuperFluxExtractor for
  more details.
 * 
 * Category: Rhythm
 * Mode: standard
 */
class SuperFluxNovelty extends BaseAlgorithm
{
    protected string $algorithmName = 'SuperFluxNovelty';
    ...
    public function compute($input): array
    {
        try {
            ...
            throw new AlgorithmException(
                "Failed to compute SuperFluxNovelty: " . $e->getMessage(),
                0,
                $e
            );
        }
    ...
    private function initializeAlgorithm(): void
    {
        ...
        try {
            ...
            if (!$this->algorithmHandle) {
                throw new AlgorithmException("Failed to create SuperFluxNovelty algorithm instance");
            }
            ...
            throw new AlgorithmException("FFI error initializing SuperFluxNovelty: " . $e->getMessage(), 0, $e);
        }
    }
