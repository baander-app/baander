...
import { isDev } from '../../shared/env.ts';
...
export function registerWasmProtocol(): void {
  protocol.handle(PROTOCOL, async (request) => {
    ...
    // In development, serve from electron/public
    // In production, serve from Resources directory
    let fullPath: string;

    if (process.env.VITE_DEV_SERVER_URL) {
      // Development: serve from electron/public
      fullPath = path.join(process.cwd(), 'electron/public', relativePath);
    } else {
