class WasmSpectrumProcessor extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    // Buffer sizes - keep full resolution to ensure all bands are represented
    this._monoAcc = new Float32Array(2048);
    this._monoIdx = 0;

    // Full-resolution output buffers for better visualization
    this._freqBins = 512;  // Full resolution for better visualization
    this._timeBins = 512;
    this._freqOut = new Uint8Array(this._freqBins);
    this._timeOut = new Uint8Array(this._timeBins);

    // More frequent posting for responsive display
  ...
  process(inputs, outputs) {
    ...
    // Accumulate to full buffer for better frequency resolution
    ...
    if (this._monoIdx >= 2048) {
      ...
      // Copy the full magnitude data for better frequency resolution
      ...
      // Take the first 512 frequency values directly
      for (let i = 0; i < this._freqBins; i++) {
        this._freqOut[i] = mag[i];
      }

      // Downsample time domain data (not as critical for visualization)
      for (let i = 0; i < this._timeBins; i++) {
        this._timeOut[i] = wav[i * 4];
      }

      // Post more frequently for responsiveness
